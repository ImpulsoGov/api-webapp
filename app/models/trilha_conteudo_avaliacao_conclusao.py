...
class AvaliacaoConclusaoConteudo(Base):
    ...
    usuario_id = Column(UUID(as_uuid=True),nullable=False, default=uuid.uuid4,comment='Chave de Identificação Unica do usuario')   
    codigo_conteudo = Column(String,nullable=False,comment='Codigo do conteudo')
    avaliacao = Column(Integer,nullable=True,comment='Avaliação do usuario do conteudo 1 a 5 estrelas')
