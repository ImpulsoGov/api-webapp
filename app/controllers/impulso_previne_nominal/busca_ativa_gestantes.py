...
def consulta_gestantes_equipe(municipio_uf, equipe):
    try:
        ...
        res_tupla = session.execute(
            "SELECT * FROM impulso_previne_dados_nominais.painel_enfermeiras_lista_nominal_gestantes where municipio_uf='"
            + municipio_uf
            + "'"
            + " and equipe_ine ='"
            + equipe
...
def consulta_gestantes_coordenacao(municipio_uf):
    try:
        ...
        # if type(item.__dict__[key]) == bool: item.__dict__[key]=int(item.__dict__[key])
        res_tupla = session.execute(
            "SELECT * FROM impulso_previne_dados_nominais.painel_coordenadores_lista_nominal_gestantes where municipio_uf='"
            + municipio_uf
            + "';"
        ).fetchall()
        res = []
...
def cadastros_duplicados_gestantes_por_equipe(municipio_uf, equipe):
    try:
        res_tupla = session.execute(
            "SELECT * FROM impulso_previne_dados_nominais.painel_cadastros_gestantes_duplicadas where municipio_uf='"
            + municipio_uf
            + "'"
            + " and equipe_ine ='"
            + equipe
