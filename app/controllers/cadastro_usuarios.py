...
def validar_senha(senha):
    ...
    if len(senha) < 9:
        restricoes.append(
            {"mensagem1": "Senha deve conter ao menos 8 caracteres"}
        )
    ...
    if len(re.findall("\W", senha)) == 0:
        restricoes.append(
            {"mensagem4": "Senha deve conter caracteres especiais"}
        )
    if len(re.findall("\s", senha)) != 0:
        restricoes.append(
            {"mensagem4": "Senha não deve conter espaços e quebras"}
        )
...
def obter_id(email):
    try:
        ...
        return (
            res[0].id
            if len(res) != 0
            else {"mensagem": "E-mail não cadastrado"}
        )
    except:
...
def cadastro_ip(municipio, cargo, telefone, whatsapp, mail, equipe):
    try:
        ...
        session.rollback()
        return {
            "mensagem": "Validação dos dados enviados não efetuada",
            "error": True,
        }
    try:
        usuario_dados = usuarios_ip.UsuarioIP(
...
def ativar_perfil(id_cod, id):
    ...
    try:
        usuario_id = (
            session.query(usuarios.Usuario)
            .filter_by(**id_db)
            .all()[0]
            .perfil_ativo
        )
        ...
        if usuario_id != None:
            return {
                "mensagem": "Usuário já realizou primeira ativação",
                "error": True,
            }
    except Exception as error:
        session.rollback()
...
def cadastrar_em_lote(
    ...
    if cad_impulso["error"] == None:
        cad_ip = cadastro_ip(
            municipio_uf, cargo, telefone, whatsapp, mail, equipe
        )
