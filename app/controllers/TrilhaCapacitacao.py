...
def consulta_avaliacao_conclusao(usuario_id,codigo_conteudo):
    try:
        query = db.session.query(trilha_conteudo_avaliacao_conclusao.AvaliacaoConclusaoConteudo).filter_by(usuario_id=usuario_id,codigo_conteudo=codigo_conteudo)
        res = query.all()
...
def avaliacao_conteudo(usuario_id : str,codigo_conteudo : str,avaliacao : int):
    ...
    if res['error'] != None : return {"msg": res['error'], "error": True}
    ...
    def avaliacao_check(avaliacao):
        if avaliacao in range(1,5) : return int(avaliacao)
        return False
    if avaliacao_check(avaliacao)==False : return {"error":"Avaliação invalida, informe um numero de 1 a 5"}
    if len(res['data']) < 1 :
        ...
        try:
            ...
            session.rollback()
            return {"error":error}
    if res['data'][0].__dict__["avaliacao"] == True : return {"msg":"Conteudo já avaliado","concluido":True}
    if res['data'][0].__dict__["avaliacao"] == None :
        try:
            session.query(trilha_conteudo_avaliacao_conclusao.AvaliacaoConclusaoConteudo).filter(
                trilha_conteudo_avaliacao_conclusao.AvaliacaoConclusaoConteudo.usuario_id == usuario_id,
                trilha_conteudo_avaliacao_conclusao.AvaliacaoConclusaoConteudo.codigo_conteudo == codigo_conteudo,
...
def conclusao_conteudo(usuario_id : str,codigo_conteudo : str,concluido : bool):
    ...
    if len(res['data']) < 1 :
        ...
        try:
            ...
            session.rollback()
            return {"error":error}
    if res['data'][0].__dict__["concluido"] == True : return {"msg":"Conteudo já concluido","concluido":True}
    if res['data'][0].__dict__["concluido"] == None :
        try:
            ...
            session.commit()
            return {"msg":"Conclusão de conteudo cadastrada com sucesso","error":None}
        except Exception as error: 
            session.rollback()
            return {"error":error}
