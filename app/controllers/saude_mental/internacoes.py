...
from app.models.saude_mental.internacoes import (
    ...
    InternacaoRelacaoRapsResumoAltas12m,
...
def obter_internacoes_raps_resumo_altas_12m_por_id_sus(
    municipio_id_sus: str,
):
    internacoes_raps_resumo = (
        session.query(InternacaoRelacaoRapsResumoAltas12m)
        .filter_by(unidade_geografica_id_sus=municipio_id_sus)
        .all()
    )
    ...
    if len(internacoes_raps_resumo) == 0:
        raise HTTPException(
            status_code=404,
            detail=(
                "Resumo de internações por altas ",
                "do município não encontrado",
            ),
        )
...
def obter_internacoes_raps_resumo_admissoes_12m_por_id_sus(
    municipio_id_sus: str,
):
    internacoes_raps_resumo = (
        session.query(InternacaoRelacaoRapsResumoAdmissoes12m)
        .filter_by(unidade_geografica_id_sus=municipio_id_sus)
        .all()
    )
    ...
    if len(internacoes_raps_resumo) == 0:
        raise HTTPException(
            status_code=404,
            detail=(
                "Resumo de internações por admissões ",
                "do município não encontrado",
            ),
        )
