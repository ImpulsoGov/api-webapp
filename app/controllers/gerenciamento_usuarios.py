...
def cargo_nome(id_cod, id):
    ...
    try:
        perfil = (
            ...
            .join(UsuariosIP)
            .with_entities(
                func.array_agg(func.distinct(Perfil_lista.perfil)).label(
                    "perfis"
                ),
...
def desativar_usuario(id_cod, id, username, acesso):
    ...
    if res[0].perfil_ativo == True:
        try:
            id_db = {"mail": id} if id_cod == 1 else {"cpf": id}
            session.query(Usuarios).filter_by(**id_db).update(
                {"perfil_ativo": False}
            )
            session.commit()
...
def primeiro_acesso(id_cod, id, perfil, username, acesso):
    ...
    if res[0].perfil_ativo != None:
        return {
            "mensagem": "Usuário já passou pela primeira liberação de perfil"
        }
    usuario_id = db.session.query(Usuarios).filter_by(**id_db).all()[0].id
    perfil_id = (
        db.session.query(Perfil_lista).filter_by(perfil=perfil).all()[0].id
    )
    novo_perfil = Perfil(
        id=str(uuid.uuid4()),
    ...
    try:
        ...
        codigo_ativ_usuario = Ativar(
            codigo_ativacao=codigo, usuario_id=usuario_id
        )
...
def primeira_ativacao_em_lote(id_cod, id, username, acesso):
    ...
    try:
        usuario_id = (
            db.session.query(Usuarios).filter_by(**id_db).all()[0].perfil_ativo
        )
        print(usuario_id)
        if usuario_id != None:
...
def consulta_primeiro_acesso(mail):
    try:
        res = db.session.query(Usuarios).filter_by(mail=mail).all()
        if len(res) < 1:
            return {"mensagem": "E-mail não cadastrado", "success": False}
        mail_db = res[0].mail
        if mail_db == None or mail != mail_db:
            return {"mensagem": "E-mail não cadastrado", "success": False}
...
def criar_codigo_recuperacao(mail):
    res = (
        db.session.query(recuperacao_senha.Recuperar).filter_by(mail=mail).all()
    )
...
def validar_codigo(codigo, mail):
    try:
        res = (
            db.session.query(recuperacao_senha.Recuperar)
            .filter_by(mail=mail)
            .all()
        )
        if len(res) < 1:
