...
def solicita_recuperacao(usuario_mail):
    ...
    try:
        ...
        if consulta == True:
            ...
            exemplo = ""
            mensagem = (
                'Codigo de recuperação (campo : "codigo") '
                + codigo
                + " envie o codigo no endpoint "
                + url
                + ' junto com e-mail cadastrado (campo : "mail") e nova senha (campo : "senha")  no corpo (form-data) da requisição, exemplo da requisição no endereço '
                + exemplo
            )
            enviar_mail(usuario_mail, mensagem)
            return {
                "mensagem": "Solicitação efetuada com sucesso,você ira receber no e-mail cadastrado o codigo de recuperação e endpoint de cadastrar nova senha"
            }
    except Exception as error:
        print({"error": error})
        session.rollback()
...
def recuperar(usuario_mail, codigo_recuperacao, nova_senha):
    ...
    query = session.query(Recuperar).filter_by(
        mail=usuario_mail, codigo_recuperacao=codigo_recuperacao
    )
    res = query.all()
    # validar senha
    ...
    # inserir a hash da nova senha no banco
        ...
        session.query(Usuario).filter(Usuario.mail == usuario_mail).update(
            {"hash_senha": hash}
        )
