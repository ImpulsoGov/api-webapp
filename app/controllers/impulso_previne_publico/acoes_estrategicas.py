...
from app.models.impulso_previne_publico.acoes_estrategicas_repasses import (
    AcoesEstrategicasRepasses,
)
from app.models.impulso_previne_publico.acoes_estrategicas_vigente_agrupada import (
    AcoesEstrategicasVigenteAgrupada,
)
...
def acoes_estrategicas_repasses(municipio_uf):
    ...
    try:
        if result is None:
            result = (
                session.query(AcoesEstrategicasRepasses)
                ...
                .order_by(AcoesEstrategicasRepasses.data_inicio)
                .all()
            )
...
def acoes_estrategicas_vigente_agrupada(municipio_uf):
    ...
    try:
        if result is None:
            result = (
                session.query(AcoesEstrategicasVigenteAgrupada)
                .filter_by(municipio_uf=municipio_uf)
                .with_entities(
                ...
                .order_by(AcoesEstrategicasVigenteAgrupada.acao_nome)
                .all()
            )
