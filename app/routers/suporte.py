...
@router.get(
    "/suporte/ger_usuarios/lista-usuarios-sem-liberacao",
    response_model=Usuarios_lista,
)
async def lista_usuarios_sem_liberacao(
    username: Usuario = Depends(auth.get_current_user),
):
    res = gerenciamento_usuarios.lista_usuarios_sem_liberacao(
        username["perfil"], 2
    )
    return res
...
@router.get(
    "/suporte/ger_usuarios/lista-usuarios", response_model=Usuarios_lista
)
...
async def dados_cadastro(
    ...
    res = gerenciamento_usuarios.dados_usuarios(
        id_cod, id, username["perfil"], 2
    )
    return res
...
@router.post(
    "/suporte/ger_usuarios/perfil-primeiro-acesso", response_model=Mensagem
)
...
@router.post(
    "/suporte/ger_usuarios/ativacao-primeiro-acesso", response_model=Mensagem
)
...
@router.post(
    "/suporte/ger_usuarios/ativacao-primeiro-acesso-em-lote",
...
async def gen_chave(
    chave: str, form_data: OAuth2PasswordRequestForm = Depends()
):
...
async def alteracao_senha(
    ...
    return gerenciamento_usuarios.senha_primeiro_acesso(
        mail, codigo, nova_senha
    )
