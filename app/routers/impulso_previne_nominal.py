...
async def gestantes_cadastros_equipe(
    ...
    res = cadastros_duplicados.cadastros_duplicados_gestantes_por_equipe(
        municipio_uf, equipe
    )
    return res
...
async def gestantes_cadastros_municipio(
    ...
    res = cadastros_duplicados.cadastros_duplicados_gestantes_por_municipio(
        municipio_uf
    )
    return res
...
async def consulta_avaliacao_conclusao_conteudo(
    usuario_id: str = Form(...), username: Usuario = Depends(auth.get_current_user)
):
...
async def acesso_trilha(
    usuario_id: str, username: Usuario = Depends(auth.get_current_user)
):
    return TrilhaCapacitacao.trilha_acesso(usuario_id)


@router.get("/impulsoprevine/capacitacao/acesso-modulos-trilha")
async def acesso_modulos(
    usuario_id: str, trilha_id: str, username: Usuario = Depends(auth.get_current_user)
):
